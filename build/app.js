    ...
    initializeMiddlewares() {
        if (this.env === 'production') {
            this.app.use(morgan_1.default('combined', { stream: logger_1.stream }))
                .use(cors_1.default({ origin: 'your.domain.com', credentials: true }));
        }
        ...
        this.app.set('view engine', 'ejs')
            .set('views', path_1.join(path_1.resolve(''), '/views'))
            .use(hpp_1.default())
            .use(helmet_1.default({ contentSecurityPolicy: false }))
            .use(compression_1.default())
            .use(express_1.default.json())
            .use(express_1.default.urlencoded({ extended: true }))
            .use(cookie_parser_1.default())
            .use('/assets', express_1.default.static(path_1.join(path_1.resolve(''), 'assets')));
    }
